...
team's marks (Q10, Q26). All of them return
...
| `blueprints()` | the team's tiles carrying a blueprint — an overlay naming a building model (Q26) — as `(x, y, model)` tuples, sorted by row then column. |
| `painted(color)` | the team's tiles painted `color`, as `(x, y)` tuples, sorted by row then column. |
| `overlaid(label)` | the team's tiles carrying the overlay `label`, as `(x, y)` tuples, sorted by row then column; a blueprint is an overlay whose label is a building model (Q26). |
