...
`problems-fixed-NNN.md` shard in [history/](history/README.md), with the commit
that closed it. What stays here is the working set; what leaves is the record.
...
**derived** — open entries counted here, fixed entries counted across the
history shards — and machine-checked on every CI run. Do not hand-edit half of
it, and do not restate it anywhere else; other docs cite entries and their
relationships, never totals.

**Status 2026-08-17 (latest): 0 opened, 0 fixed — zero open.**
